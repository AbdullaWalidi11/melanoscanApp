...
export default function CompareResult() {
  ...
  const handleAddToHistory = async () => {
    try {
      ...
      navigation.replace("ComparisonHistory", { parentLesionId: oldLesionId, imageUri: oldImageUri });
    ...
    <ScrollView className="flex-1 bg-white" contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <View className="items-center px-6">
          ...
          <View 
            ...
            style={{ borderColor: statusColor, backgroundColor: isStable ? "#F0FFF4" : "#FFF0F0" }}
          >
              {/* Color Block (The Square in your Figma) */}
              <View 
                className="w-32 h-32 rounded-2xl mb-4 shadow-sm"
                style={{ backgroundColor: isStable ? "#8B4513" : "#B22222" }} 
              />
              
              <Text className="text-xl font-bold mb-1" style={{ color: statusColor }}>
      ...
      <View className="px-6 mt-10">
          ...
          <Text className="text-gray-400 text-xs text-center mt-6 px-4">
              This result is not a diagnosis. Please consult a doctor for accurate diagnosis.
          </Text>
      </View>
