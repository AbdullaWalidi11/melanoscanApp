...
export default function Dropdown({ label, selectedValue, onValueChange, options }: DropdownProps) {
      ...
      {isOpen && (
        <View className="bg-gray-50 border border-gray-200 mt-1 rounded-xl overflow-hidden">
          {options.map((option) => (
            ...
            >
              <Text className={selectedValue === option ? "text-[#e2728f] font-bold" : "text-gray-800"}>
                {option}
              </Text>
            </TouchableOpacity>
          ))}
